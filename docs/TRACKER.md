...
| H-2 | Google Cloud OAuth client (Testing mode, test users, Sheets API enabled, both redirect URIs); `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` in the root `.env` | done | 2026-09-23, localhost and https://autoapps.win redirect URIs; production and local share this client |
...
| H-4 | Vercel project for `frontend/` with env vars and `NEXTAUTH_URL=https://autoapps.win`; domain `autoapps.win` attached | done | 2026-09-23; Framework Preset had to be set to Next.js; www.autoapps.win is primary (apex redirects to it) |
...
| P2-5 | Builder-side app routes (create, get, messages, publish, restore, activity, patch, delete) | must | P2-4, P1-5 | done | b287e60 | curl flow passed; non-editors get published only + 403 on edits; failed edit → assistant message, version kept |
...
| P6-3 | Demo rehearsal checklist | should | P6-1 | done | 059b1e5 | checklist in docs/DEMO.md; `npm run reset:demo` clears a person's fill-in cells (dry run first). The two rehearsals themselves need Amos + the H-6 consumer account |
...
- 2026-09-23 · Claude Opus 5.5 · Outside the plan, at Amos's request: fixed the cookie banner reappearing in the side panel (c95268e), and rebuilt the signed-out homepage as a SaaS landing page with Amos's demo video, click-to-play (0a924bf). Checked in Chrome at desktop width and 390 px.
- 2026-09-23 · Claude Opus 5.5 · Phase 6 done: P6-1 (deploy, done during the switch to production), P6-2 (light theme, empty states and errors incl. panel clipboard, one icon everywhere, README), P6-3 (docs/DEMO.md plus `npm run reset:demo`). P5-1/P5-2 done: the badge was verified on a real sheet through Chrome automation. P5-3 is blocked only on a 30-second human check, because automation cannot see the side panel. Remaining human items are under Needs Amos: the P5 panel check, and H-6 (a second same-domain account) for the P0-4 real check and the two-person rehearsals.
